...
class AnimeFLV(object):
    ...
    def animes_emision(self):
        response = requests.get(BASE_URL)

        # Verificar si la solicitud fue exitosa
        if response.status_code == 200:
            List_emi = []
            # Cargar el contenido HTML en BeautifulSoup
            soup = BeautifulSoup(response.text, 'html.parser')

            # Buscar todos los elementos <a> que contienen los nombres de los animes
    ...
    def filter(self, geners: str = None, page: int = None) -> List[AnimeInfo]:
        """
        ...
        if geners is not None:
            params["geners"] = geners
        if page is not None:
            params["page"] = page
        
        params = urlencode(params)

        url = f"https://www3.animeflv.net/browse?genre[]={geners}&page={page}"
